        ...
        for _ in 1...100 {
            ...
            for _ in 1...50 {
                ...
                if userChoseCorrectLetter {
                    gameBrain.letterSelected(letterSelected: gameBrain.targetLetter)
                    XCTAssertEqual(gameBrain.score, oldScore + 1)
                } else {
                    let filteredArray = gameBrain.alphabet.filter { $0 != gameBrain.targetLetter }
                    let randomLetter = filteredArray.randomElement()!
                    gameBrain.letterSelected(letterSelected: randomLetter)
